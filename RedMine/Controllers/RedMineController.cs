...
namespace RedMine.Controllers
{
	public class RedMineController : ApiController
	{
		...
		public HttpResponseMessage updateDownTxt()
		{
			try
			{
				...
				do
				{
					...
					HtmlNodeCollection hnc = hDoc.DocumentNode.SelectNodes("//table[@class='list issues odd-even sort-by-id sort-desc']/tbody/tr");
					if (hnc == null)
					{
						isLastPage = true;
						break;
		...
		public JArray getVersionCombo(object data)
		{
			...
			HtmlDocument hDoc = new HtmlDocument();
			hDoc.LoadHtml(scraper.Html);
			HtmlNodeCollection hnc = hDoc.DocumentNode.SelectNodes("//h3[@class='icon icon-package version inline-flex']");
			foreach (HtmlNode hn in hnc)
			{
				result = new JObject();
